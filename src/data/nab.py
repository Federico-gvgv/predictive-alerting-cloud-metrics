...
2. **Parse** a single CSV file (or a small set) from NAB together with
   the corresponding anomaly-window labels from
   ``labels/combined_windows.json``.
3. Return a **unified DataFrame** with columns
   ``["timestamp", "value", "is_incident"]``.

By default only **one** NAB file is loaded (configured via
``dataset.nab.nab_file``) so that the resulting time series is a single
contiguous signal.  Loading multiple files into one timeline can
interleave independent services and create invalid sliding windows.
"""
...
def _load_single_csv(
    csv_path: Path,
    label_key: str,
    all_windows: dict[str, list[tuple[str, str]]],
) -> pd.DataFrame:
    """Load one NAB CSV and attach incident labels."""
...
def load_nab(cfg: dict[str, Any]) -> pd.DataFrame:
    """Load a NAB CSV and return a single-series DataFrame.
    ...
    cfg:
        ...
        * ``nab_file``       – relative path within ``data/`` such as
          ``"realKnownCause/ambient_temperature_system_failure.csv"``
          (default).  When set, **only this file** is loaded to guarantee
          a single contiguous time series.
    ...
    nab_file: str | None = nab_cfg.get("nab_file", None)

    if nab_file:
        # ── Single-file mode (default) ───────────────────────────────
        csv_path = nab_root / "data" / nab_file
        if not csv_path.is_file():
            raise FileNotFoundError(
                f"NAB file not found: {csv_path}. "
                f"Check dataset.nab.nab_file in config."
            )

        logger.info("Loading single NAB file: %s", nab_file)
        result = _load_single_csv(csv_path, nab_file, all_windows)

    else:
        # ── Legacy multi-file mode (kept for backwards compatibility) ─
        ...
        if not subset_dir.is_dir():
            available = sorted(
                p.name for p in (nab_root / "data").iterdir() if p.is_dir()
            )
            raise FileNotFoundError(
        ...
        logger.info(
            "Loading %d NAB CSV(s) from subset '%s'.", len(csv_files), subset
        )
        ...
        for csv_path in csv_files:
            ...
            frames.append(
                _load_single_csv(csv_path, label_key, all_windows)
            )
