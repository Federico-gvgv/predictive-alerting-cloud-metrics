...
* **Event recall** – fraction of incident intervals where ≥ 1 alert fires
  strictly *before* the incident starts.
...
def event_metrics(
    ...
    *before* the incident's ``start_ts``.  The alert is matched to the
    nearest *future* incident (greedy, first-come first-served).
    ...
    dict with:
        ...
        fp_count, fp_per_10k,
        lead_times (list of timedeltas as seconds),
        lead_time_median_s, lead_time_iqr_s,
    """
    ...
    if n_incidents == 0:
        return {
            ...
            "lead_time_iqr_s": float("nan"),
        }

    # For each incident, find earliest alert that fires before start
    detected = [False] * n_incidents
    lead_times: list[float] = []  # seconds
